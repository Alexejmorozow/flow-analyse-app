...
# ... (restliche bestehende Funktionen beibehalten: get_all_data, reset_database, create_team_analysis) ...
    ...
    *Bewerten Sie für jede Domäne:*  
    ...
    if st.button("🚀 Theoriegestützte Analyse starten", disabled=not confirmed):
        ...
        with st.spinner('Analysiere Daten und erstelle Report...'):
            ...
            if st.button("🤖 KI-Analyse generieren", key="generate_ai_analysis"):
                    ...
                    for domain in DOMAINS:
                        with st.expander(f"🧠 {domain}", expanded=False):
                            ...
                            if analysis:
                                ...
                                fallback = get_fallback_domain_analysis(current_data, domain)
                                st.session_state.ai_analysis[domain] = fallback
                                st.warning("⚠️ KI nicht verfügbar - Fallback-Analyse:")
                                st.write(fallback)
            
            # Zeige gespeicherte KI-Analysen falls vorhanden
            if st.session_state.ai_analysis:
                for domain, analysis in st.session_state.ai_analysis.items():
                    with st.expander(f"📖 {domain} (gespeicherte Analyse)", expanded=False):
                        st.write(analysis)
            
            # 4. Vollständiger KI-Report
            st.subheader("📄 Vollständiger KI-Report")
            
            if st.button("📊 Kompletten KI-Report erstellen", key="generate_full_report"):
                with st.spinner('KI erstellt umfassenden Bericht...'):
                    ai_report = generate_comprehensive_ai_report(current_data)
                    
                    if ai_report:
                        st.text_area("KI-Report", ai_report, height=400)
                        st.download_button(
                        ...
                        st.warning("KI-Report konnte nicht generiert. Bitte API-Key prüfen.")

            # 5. Entwicklungsplan (unverändert)
    ...
    # Team-Analyse Funktion aufrufen (unverändert)
